...
function Form (props: FormPropsFull<Record<string, any>>) {
	...
	const renderFormGroupItem = (input: FormSchemeInputFull) => {
		...
		if (type.match(/(select|radio)/)) items.forEach((item) => (labels[item.value] = item.label));
		...
		switch (type) {
					...
					<Select
						...
						renderValue={(selected: string[]) => {
							return selected.length === 0 ? (
								...
								selected.filter((selected) => selected !== '').map((selected) => labels[selected]).join(', ')
							);
						}}
						{...common_props}
						{...input_props}
			...
			case 'checkbox':
				return <Field as={Checkbox} name={full_path} type={'checkbox'} color={'primary'} {...input_props} />;
			case 'switch':
				return <Field as={Switch} name={full_path} type={'checkbox'} color={'primary'} {...input_props} />;
			case 'radio':
				return (
			...
			case 'number':
				return <TextField type={'number'} fullWidth {...common_props} {...input_props} />;

			default:
				return <TextField type={'text'} multiline={type === 'textarea'} fullWidth {...common_props} {...input_props} />;
		}
	};
	...
	const renderFormGroup = (input: FormSchemeInputFull) => {
		...
		if (disabled && required) throw new Error('Required fields cannot be disabled');
		return (
			<FormControl
			...
			>
				<div className="FormScheme-input-label_helpertext" style={{ marginRight: 10 }}>
					<FormLabel
						style={{
							display: 'flex',
							fontWeight: 'bold',
				...
				<div className="Formscheme-input-container" style={{ marginBottom: show_error ? 0 : 22.5 }}>
					{type === 'group' ? treeView ? (
						<TreeView
							...
							onNodeToggle={(e) => {
								...
								(e.target as any).textContent = !parent.nextElementSibling ? 'Collapse' : 'Expand';
							}}
						>
							<TreeItem
								nodeId="1"
								label={<div style={{ width: 'calc(100% - 25px)' }}>{collapse ? 'Expand' : 'Collapse'}</div>}
							>
								<FormGroup row={false}>{children.map((child) => renderFormGroup(child))}</FormGroup>
							</TreeItem>
						</TreeView>
					) : (
						<FormGroup row={true}>{children.map((child) => renderFormGroup(child))}</FormGroup>
					) : (
	...
	return (
		...
		>
			...
			<div className={`Formscheme-buttons`} style={{ display: 'flex', justifyContent: formButtonsPlacement }}>
				{formButtons && (
					<FormGroup
						row={true}
						style={{
