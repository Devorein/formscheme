...
class InputForm extends Component {
	populateInitialValue = (inputs, { validateOnChange, validationSchema }) => {
		...
		function inner (input, parents = [], index) {
			let target_obj = {};
			const full_path = parents.reduce((acc, parent) => (parent.append ? initialValues[parent.name] : acc), []);
			const parent = parents[parents.length - 1] || {};
			const { extra = {} } = parent;
			if (input.type === 'group') {
				input.children.forEach((child, index) => inner(child, [ ...parents, input ], index));
			} else {
